...
namespace geodesic
{
	...
	{
	public:
		GeodesicAlgorithmExact(geodesic::Mesh *mesh) : GeodesicAlgorithmBase(mesh),
		...
		void propagate_landmark(std::vector<SurfacePoint> &sources,
								std::vector<GeodesicAlgorithmSubdivision *> *landmarks,
	...
	private:
		...
		void update_list_and_queue_landmark(list_pointer list,
	...
	inline void GeodesicAlgorithmExact::propagate_landmark(std::vector<SurfacePoint> &sources,
														   std::vector<GeodesicAlgorithmSubdivision *> *landmarks,
		...
		while (!m_queue.empty())
		{
			...
			for (unsigned i = 0; i < edge->adjacent_faces().size(); ++i) // two possible faces to propagate
			{
				...
				if (num_propagated)
				{
					...
					update_list_and_queue_landmark(interval_list(next_edge),
												   candidates,
												   num_propagated,
												   landmarks,
												   pstop);
	...
	inline void GeodesicAlgorithmExact::update_list_and_queue_landmark(list_pointer list,
																	   IntervalWithStop *candidates, // up to two candidates
																	   unsigned num_candidates,
																	   std::vector<GeodesicAlgorithmSubdivision *> *landmarks,
																	   SurfacePoint pstop)
	{
		...
		for (int i = 0; i < landmarks->size(); i++)
		{
			...
			if (dt < de + edge->length() && dt > de - edge->length())
				continue;
		...
		if (list->first() == NULL)
		{
			...
			if (num_candidates == 1)
			{
				first = candidates;
				second = candidates;
				first->compute_min_distance_landmark(first->stop(), dist);
			}
			else
			{
				...
				first->compute_min_distance_landmark(first->stop(), dist);
				second->compute_min_distance_landmark(second->stop(), dist);
			}
			...
			while (p != NULL && p->start() < q->stop() - local_epsilon) // go through all old intervals
			{
				...
				if (N == 1)
				{
					if (map[0] == OLD) // if "p" is always better, we do not need to update anything)
					{
						...
						{
							previous->next() = p;
							previous->compute_min_distance_landmark(p->start(), dist);
				...
				if (map[0] == OLD) // finish previous, if any
				{
					...
					{
						previous->next() = p;
						previous->compute_min_distance_landmark(previous->stop(), dist);
						m_queue.insert(previous);
						previous = NULL;
					}
					i_new[0] = p;
				...
				for (unsigned j = 0; j < N; ++j) // find "min" and add the intervals to the queue
				{
					...
					else
					{
						...
						current_interval->compute_min_distance_landmark(current_interval->stop(), dist); // compute minimal distance
																	   ...
																	   interval_pointer source_interval) // up to two candidates
	{
		...
		Interval::DirectionType direction = edge->adjacent_faces()[0]->id() == face->id() ? Interval::FROM_FACE_0 : Interval::FROM_FACE_1;
	...
	inline double GeodesicAlgorithmExact::get_memory()
	{
		...
		double memory_usage = m_edge_interval_lists.size() * sizeof(IntervalList) +
